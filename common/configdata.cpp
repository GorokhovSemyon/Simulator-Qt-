...
ConfigData::ConfigData(const QString & config, QObject *parent): QObject(parent),standardOutput(stdout),
    dataFile(config),jmax(0)
{
    QString readData;
...
ConfigData::ConfigData(const QString & config, const QString & name, QObject *parent) : QObject(parent),
    standardOutput(stdout), dataFile(config),jmax(0)
{
    QString readData;
        ...
        else
        {
            ...
            if (tmpstr.contains("device"))
            {
                if (tmpstr.contains("frequency"))
                    data.device_frequency = (float)(tmp.mid(0, endpos)).toFloat();
                else
            ...
            if (tmpstr.contains("receiver"))
            {
                if (tmpstr.contains("ip"))
                    ...
                    if (tmpstr.contains("port"))
                        ...
                        if (tmpstr.contains("frequency"))
                            data.receiver_frequency = (float)(tmp.mid(0, endpos)).toFloat();
            }
            if (tmpstr.contains("sender"))
            {
                if (tmpstr.contains("ip"))
                    ...
                    if (tmpstr.contains("port"))
                        ...
                        if (tmpstr.contains("frequency"))
                            data.sender_frequency = (float)(tmp.mid(0, endpos)).toFloat();
            }
